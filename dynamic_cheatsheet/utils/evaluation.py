...
def eval_for_exact_matching_with_no_punctuation(output: str, target: str) -> bool:
    """
...
def eval_for_CheckmateInOne(input: str, output: str, target: str) -> bool:
    ...
    if next_move_idx not in output:
        if target in output or (
            target[1] == "x" and (target[0] + target[2:]) in output
        ):
            return True
    else:
...
def eval_for_multiple_choice(input_text: str, final_answer: str, target: str) -> bool:
            ...
            if not options_section:
                ...
                for i, line in enumerate(lines):
                    if line.strip().startswith(
                        f"({option_letter})"
                    ) or line.strip().startswith(f"{option_letter})"):
                        return line.split(")", 1)[1].strip()

            # Process the options section if found
            for line in options_section.split("\n"):
                line = line.strip()
                if line.startswith(f"({option_letter})") or line.startswith(
                    f"{option_letter})"
                ):
                    return line.split(")", 1)[1].strip()
    ...
    if (
        len(clean_answer) == 1
        and clean_answer in "abcd"
        and clean_answer == target_letter
    ):
        return True

    # Handle answer formats like "A" or "A."
    if clean_answer.startswith(target_letter) and (
        len(clean_answer) == 1 or (len(clean_answer) == 2 and clean_answer[1] == ".")
    ):
        return True

    # Handle answer formats like "Option A" or "Answer is A"
    if clean_answer.endswith(target_letter) and (
        clean_answer[-2:] == f" {target_letter}"
        or clean_answer[-3:] == f" {target_letter}."
    ):
        return True
