...
def main(args: Arguments):
    ...
    if args.task in PREDEFINED_PROMPTS and args.task != "P3_Test":
        ...
        raise ValueError(
            f"Task {args.task} is not recognized. Please make sure the task name is correct."
        )
    ...
    if args.continue_from_last_run_path:
        if not os.path.exists(args.continue_from_last_run_path):
            raise ValueError(
                f"The provided path {args.continue_from_last_run_path} does not exist."
            )

        # Read the previous run parameters from the previous run file and compare them with the provided arguments
        previous_run_param_path = args.continue_from_last_run_path.replace(
            ".jsonl", "_params.json"
        )
        ...
        for key in args_keys:
            if getattr(args, key) != previous_run_params[key]:
                raise ValueError(
                    f"Warning: The provided argument {key} is inconsistent with the previous run. The previous run value is {previous_run_params[key]}."
                )

        # Create a new save path name based on the previous run path
        args.save_path_name = args.continue_from_last_run_path.replace(
            ".jsonl", "_continued.jsonl"
        )
    else:
    ...
    if args.continue_from_last_run_path:
        ...
        # Print the details
        print(
            f"Continuing from the previous run at {args.continue_from_last_run_path}."
        )
    ...
    for idx, example in enumerate(dataset):
        ...
        if args.task in PREDEFINED_PROMPTS:
            input = (
                f"{PREDEFINED_PROMPTS[args.task]}\n\nQuestion #{idx + 1}:\n{orig_input}"
            )
        else:
        ...
        if (
            args.task == "AIME_2020_2024"
            or args.task == "AIME_2024"
            or args.task == "AIME_2025"
        ):
        ...
        if args.task == "GameOf24":
            ...
            result = eval_for_exact_matching_with_no_punctuation(
                final_answer.lower(), original_target.lower()
            )
        elif args.task in ["GPQA_Diamond", "MMLU_Pro_Engineering", "MMLU_Pro_Physics"]:
            result = result = eval_for_multiple_choice(
                input, final_answer, original_target
            )
        elif args.task == "MathEquationBalancer":
